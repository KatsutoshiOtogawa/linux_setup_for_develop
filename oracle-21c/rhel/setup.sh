...
function main {
  ...
  local os
  if cat /etc/redhant-release | grep 7. > /dev/null; then
    os=rhel7
  elif cat /etc/redhat-release | grep 8. > /dev/null; then
    os=rhel8
  else
    echo "oracle 21c is rhel 7 or 8 only install!" >&2
    return 1
  fi
  if [ ! -d ./.cache ]; then
    mkdir ./.cache
  ...
  if [ ! ${os} = "rhel7" ]; then
    if [ ! -f ./.cache/oracle-database-preinstall-21c*el7* ]; then
      ...
      wget https://yum.oracle.com/repo/OracleLinux/OL7/appstream/x86_64/getPackage/oracle-database-preinstall-21c-1.0-1.el8.x86_64.rpm -P ./.cache
    fi
