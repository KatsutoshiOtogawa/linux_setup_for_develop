...
function main {
  ...
  if cat /etc/fedora-release | grep 33 > /dev/null; then
    ...
    echo "oracle 21c is fedora33, 34, 35 only install!" >&2
    return 1
  fi
  if [ ! -d ./.cache ]; then
    mkdir ./.cache
  ...
  # preinstall el7なら下が必要。
  # http://mirror.centos.org/centos/7/os/x86_64/Packages/policycoreutils-python-2.5-34.el7.x86_64.rpm
  # dnf install -y policycoreutils-python-utils
  # package名が変わっているから、怒られるがあってる。- nothing provides policycoreutils-python needed by oracle-database-preinstall-21c-1.0-1.el7.x86_64
  # preinstall el8なら下が必要
  # https://centos.pkgs.org/8/centos-appstream-x86_64/compat-openssl10-1.0.2o-3.el8.x86_64.rpm.html
  # compat-openssl10
  if [ ! -f ./.cache/compat-openssl10-1.0.2o-3.el8.x86_64.rpm ]; then
    wget http://mirror.centos.org/centos/8-stream/AppStream/x86_64/os/Packages/compat-openssl10-1.0.2o-3.el8.x86_64.rpm -P ./.cache
  fi
  dnf -y install ./.cache/compat-openssl10-1.0.2o-3.el8.x86_64.rpm
  ...
  #   wget https://yum.oracle.com/repo/OracleLinux/OL7/latest/x86_64/getPackage/oracle-database-preinstall-21c-1.0-1.el7.x86_64.rpm -P ./.cache
  # fi
  if [ ! -f ./.cache/oracle-database-preinstall-21c*el8* ]; then
    wget https://yum.oracle.com/repo/OracleLinux/OL8/latest/x86_64/getPackage/oracle-database-preinstall-21c-1.0-1.el8.x86_64.rpm -P ./.cache
  fi

  # package名が変わっているから、怒られるがあってる。- nothing provides policycoreutils-python needed by oracle-database-preinstall-21c-1.0-1.el7.x86_64
  # dnf -y install ./.cache/oracle-database-preinstall-21c*el7*.rpm --skip-broken
  ...
  # rm ./.cache/oracle-database-preinstall-18c*
  # rm ./.cache/oracle-database-xe-18c*
}
