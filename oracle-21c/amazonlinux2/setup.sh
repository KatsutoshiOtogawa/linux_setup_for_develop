...
function main {
  ...
  local os
  if cat /etc/os-release | grep "Amazon Linux 2" > /dev/null; then
    os=amazonlinux2
  elif cat /etc/os-release | grep "Amazon Linux 2022" > /dev/null; then
    os=amazonlinux2022
  else
    echo "oracle 21c is Amazon Linux 2 only install!" >&2
    return 1
  fi
  if [ ! -d ./.cache ]; then
    mkdir ./.cache
  ...
  # oracle-21c-preinstall-el7 require in fedora, amazonlinux2, centos7
  ...
  # if [ ! -f ./.cache/oracle-database-preinstall-21c* ]; then
  #   wget https://yum.oracle.com/repo/OracleLinux/OL8/appstream/x86_64/getPackage/oracle-database-preinstall-21c-1.0-1.el8.x86_64.rpm -P ./.cache
  # fi

  # dnf -y install ./.cache/oracle-database-preinstall-21c*.rpm

  if [ ! ${os} = "amazonlinux2" ]; then
    if [ ! -f ./.cache/oracle-database-preinstall-21c*el7* ]; then

      wget https://yum.oracle.com/repo/OracleLinux/OL7/appstream/x86_64/getPackage/oracle-database-preinstall-21c-1.0-1.el7.x86_64.rpm -P ./.cache
    fi
    yum -y install ./.cache/oracle-database-preinstall-21c*el7*.rpm
  elif [ ! ${os} = "amazonlinux2022" ]; then
    if [ ! -f ./.cache/oracle-database-preinstall-21c*el8* ]; then

      wget https://yum.oracle.com/repo/OracleLinux/OL8/appstream/x86_64/getPackage/oracle-database-preinstall-21c-1.0-1.el8.x86_64.rpm -P ./.cache
    fi
    dnf -y install ./.cache/oracle-database-preinstall-21c*el8*.rpm
  fi
  ...
  if [ ! ${os} = "amazonlinux2" ]; then
    ...
    yum -y localinstall ./.cache/oracle-database-xe-21c*ol7*.rpm > /xe_logs/XEsilentinstall.log 2>&1
  elif [ ! ${os} = "amazonlinux2022" ]; then
  ...
  # if [ ! -f ./.cache/oracle-database-xe-21c* ]; then
  #   wget https://download.oracle.com/otn-pub/otn_software/db-express/oracle-database-xe-21c-1.0-1.ol8.x86_64.rpm -P ./.cache
  # fi
  # echo finish downloading oracle database!
  # echo installing oracle database...
  # dnf -y localinstall ./.cache/oracle-database-xe-21c*.rpm > /xe_logs/XEsilentinstall.log 2>&1
