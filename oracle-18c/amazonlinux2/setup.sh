...
function main {
  ...
  if ! cat /etc/os-release | grep "Amazon Linux 2" > /dev/null; then
    echo "oracle 18c is Amazon Linux 2 only install!" >&2
    return 1
  fi
  ...
  # these library needs fedora and amazonlinux2.rhel7,oracle linux are alredy installed.
  ...
  yum -y install ./.cache/oracle-database-preinstall-18c*.rpm
  ...
  yum -y localinstall ./.cache/oracle-database-xe-18c*.rpm > /xe_logs/XEsilentinstall.log 2>&1
